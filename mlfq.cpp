...
int main(){
    ...
    if(fp==NULL)
        ...
        queue <struct process> cpu1;
        queue <struct process> cpu2;
        queue <struct process> cpu3;
        ...
        while(completed != n){
            ...
            if(current_time%100==0){
                while(!cpu2.empty()){
                    cpu1.push(cpu2.front());
                    cpu2.pop();
                }
                while(!cpu3.empty()){
                    cpu1.push(cpu3.front());
                    cpu3.pop();
                }
            }
            if(!cpu1.empty()){
                cpu_occupied = true;
                int idx = cpu1.front().p_id-1;
                cpid = cpu1.front().p_id;
                struct bursts b = cpu1.front().burst.front();
                ...
                if(p[idx].burst.front().remainingBurst == 0){
                    ...
                    cpu2.push(cpu1.front());
                    cpu1.pop();
                }
            }
            else if(!cpu2.empty()){
                cpu_occupied = true;
                int idx = cpu2.front().p_id-1;
                cpid = cpu2.front().p_id;
                struct bursts b = cpu2.front().burst.front();
                if(p[idx].start_time == -1)
                    p[idx].start_time = current_time-1;
                occur.push(idx+1);
                p[idx].burst.front().remainingBurst--;
                if(p[idx].burst.front().remainingBurst == 0){
                    ...
                    cpu3.push(cpu2.front());
                    cpu2.pop();
                }
            }
            else if(!cpu3.empty()){
                cpu_occupied = true;
                int idx = cpu3.front().p_id-1;
                cpid = cpu3.front().p_id;
                struct bursts b = cpu3.front().burst.front();
                if(p[idx].start_time == -1)
                    p[idx].start_time = current_time-1;
                occur.push(idx+1);
                p[idx].burst.front().remainingBurst--;
